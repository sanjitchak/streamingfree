...
function updateCatalogStatus(){const el=$('#catalogUpdated');if(!el)return;if(!catalog.lastUpdated){el.textContent='Catalog update unavailable';return}const date=new Date(catalog.lastUpdated);const rated=media.filter(x=>x.rating?.value).length;const added=Number(catalog.summary?.newTitlesAdded||0);el.textContent=`Catalog checked ${date.toLocaleDateString(undefined,{day:'numeric',month:'short',year:'numeric'})} · ${media.length} verified titles · ${added} new this refresh${rated?` · ${rated} MAL rated`:''}`;}
