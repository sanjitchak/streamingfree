...
function renderWeek(){
  ...
  $('#weekCalendar').innerHTML=state.week.map((id,i)=>{const m=media.find(x=>x.id===id&&x.access&&x.direct)||media.find(x=>x.access&&x.direct);return `<article class="day-card"><div class="day-label"><strong>${days[i]}</strong><small>${i<5?'Weeknight':'Weekend'}</small></div><div class="day-art" data-open="${m.id}" role="link" tabindex="0" aria-label="Open ${escapeHtml(m.title)} from poster" style="background:${m.art}"><img src="${m.image}" alt="${escapeHtml(m.title)} artwork" loading="lazy" onerror="this.remove()"></div><div class="day-info"><h3>${escapeHtml(m.title)}</h3><p>${m.access} · ${m.runtime} min · ${escapeHtml(m.platform)}</p></div><button class="day-action" data-open="${m.id}" aria-label="Open ${escapeHtml(m.title)} directly">▶</button></article>`}).join('');
}
